...
```{r}
library(dplyr)
library(purrr)  # For map and map_dfr functions

# Define the function to summarize data
...
# Define the datasets and waves
datasets <- list(D = D_df, R = R_df)
waves <- c(2, 3, 4, 5, 6) # Add more waves if needed

# Create a list to store the results
results <- list()

# Loop through datasets and waves
...
```
```{r}
ggplot(differences_df, aes(x = wave, y = diff_num_non_houshold_cc)) +
  geom_line() +
  geom_point() +
  labs(
...
ggplot(differences_df, aes(x = wave, y = diff_percent_mask)) +
  geom_line() +
  geom_point() +
  labs(
...
ggplot(differences_df, aes(x = wave, y = diff_concern)) +
  geom_line() +
  geom_point() +
  labs(
