...
cd_concern_plot <- ggplot(emm_concern, aes(x = political_party_to_CD, y = emmean, color = political_party_to_CD, shape = political_party_to_CD)) +
  ...
  labs(y = "Probability of Concern", x = "", color = "Political Party", title = "Partisan Differences in Concern about Spread of the Virus \nAcross Opposing Party Districts") +
...
cd_contact_model <- glm.nb(num_cc_nonhh ~ log_current_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + r_working + resp_hhsize + r_race + cityNY +`cityBay Area` + cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + political_party_to_CD, 
                           data = bics_zip_features,
                           weights = weight_pooled)
...
cd_contact_plot <- ggplot(emm_contact, aes(x = political_party_to_CD, y = emmean, color = political_party_to_CD, shape = political_party_to_CD)) +
  geom_point(position = position_dodge(width = 0.35), size = 4) +
  scale_color_manual(name = "Political Party", values = c("blue", "purple", "magenta","red")) +
  labs(y = "Predicted Contacts", x = "", color = "Political Party", title = "Partisan Differences in Non-Household Contacts Across \nOpposing Party Districts") + 
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2, position = position_dodge(width = 0.35)) +
  theme_minimal() +
  theme(legend.position = "none",
        plot.title = element_text(size = 26),
...
mask_pred_plot <- ggplot(emm_masks, aes(x = political_party_to_CD, y = emmean, color = political_party_to_CD, shape = political_party_to_CD)) +
  geom_point(position = position_dodge(width = 0.35), size = 4) +
  scale_color_manual(name = "Political Party", values = c("blue", "purple", "magenta","red")) +
  labs(y = "Predicted Percent Masked Contacts", x = "", color = "Political Party", title = "Partisan Differences in Mask Usage Across Opposing \nParty Districts") + 
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2, position = position_dodge(width = 0.35)) +
  theme_minimal() +
  theme(legend.position = "none",
        plot.title = element_text(size = 26),
