...
columns_to_keep <- c('ResponseId','StartDate','resp_yob','resp_sex','resp_hispanic','r_race','resp_nativity','ZIP',
                     ...
                     'data_collected_dates','weight_pooled','weight_city')
...
inc_w2 <- read_csv('../data/wave_2_incidence.csv', col_types = cols(FIPS = col_character())) %>% 
  rename(COUNTY_FIPS = FIPS,
         prev_week_incidence = wave_2_incidence)

inc_w3 <- read_csv('../data/wave_3_incidence.csv', col_types = cols(FIPS = col_character())) %>% 
  rename(COUNTY_FIPS = FIPS,
         prev_week_incidence = wave_3_incidence)

inc_w4 <- read_csv('../data/wave_4_incidence.csv', col_types = cols(FIPS = col_character())) %>% 
  rename(COUNTY_FIPS = FIPS,
         prev_week_incidence = wave_4_incidence)

inc_w5 <- read_csv('../data/wave_5_incidence.csv', col_types = cols(FIPS = col_character())) %>% 
  rename(COUNTY_FIPS = FIPS,
         prev_week_incidence = wave_5_incidence)

inc_w6 <- read_csv('../data/wave_6_incidence.csv', col_types = cols(FIPS = col_character())) %>% 
  rename(COUNTY_FIPS = FIPS,
         prev_week_incidence = wave_6_incidence)
...
wave2 <- left_join(wave2, inc_w2, by = "COUNTY_FIPS") %>%
  mutate(prev_week_inc_rate = (prev_week_incidence / POPESTIMATE2020) * 100000)
...
wave5 <- left_join(wave5, inc_w5, by = "COUNTY_FIPS") %>%
  mutate(prev_week_inc_rate = (prev_week_incidence / POPESTIMATE2021) * 100000)
...
rm(wave, wave2,wave3,wave4,wave5,wave6,inc_w2,inc_w3,inc_w4,inc_w5,inc_w6)
...
cases_df <- data.frame(wave = dates, weekly_cases = weekly_cases)
...
```{r}
rm(file_type, no_mandates,strict_mandates,less_strict_mandates, unclear_or_unspecified_mandates)
```
