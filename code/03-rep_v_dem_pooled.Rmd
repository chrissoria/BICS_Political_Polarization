...
rvd_contacts_model_raw <- lm(num_cc_nonhh  ~ political_party + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, 
                                   data = bics_zip_features, 
                                   weights = weight_pooled)
...
rvd_contacts_plot <- ggplot(all_emms, aes(x = political_party, y = emmean, color = political_party, group = interaction(political_party, controls))) +
  ...
  scale_shape_manual(name = NULL, values = c("baseline" = 1, "controls" = 16)) +
  scale_size_manual(values = c("baseline" = 6, "controls" = 6)) +
...
rm(rvd_contacts_model, rvd_contacts_plot, all_emms, emm_contact, emm_contacts, emm_contacts_raw,rvd_contacts_model_raw)
```
...
```{r}
rvd_masks_model <- lm(Norm_Masks_Used  ~  log_current_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + political_party + r_working + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, 
                                   data = bics_zip_features, 
                                   weights = weight_pooled)

rvd_masks_model_raw <- lm(Norm_Masks_Used  ~ political_party + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, 
                                   data = bics_zip_features, 
                                   weights = weight_pooled)
...
Vaccines
```{r}
rvd_vaccines_model <- lm(Vaccinated  ~  log_current_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + political_party + r_working + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, 
                                   data = bics_zip_features, 
                                   weights = weight_pooled)

rvd_vaccines_model_raw <- lm(Vaccinated  ~ political_party + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, 
                                   data = bics_zip_features, 
                                   weights = weight_pooled)
...
```{r}
bi_mask_dem <- lm(Norm_Masks_Used ~ log_prev_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave + as.factor(COUNTY_FIPS), data = D_df, weights = weight_pooled)
print(summary(bi_mask_dem))

bi_mask_dem_raw <- lm(Norm_Masks_Used ~ log_prev_week_inc_rate + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, data = D_df, weights = weight_pooled)

bi_mask_rep <- lm(Norm_Masks_Used ~  log_prev_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave + as.factor(COUNTY_FIPS), data = R_df, weights = weight_pooled)
print(summary(bi_mask_rep))

bi_mask_rep_raw <- lm(Norm_Masks_Used ~ log_prev_week_inc_rate + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, data = R_df, weights = weight_pooled)

bi_mask_ind <- lm(Norm_Masks_Used ~  log_prev_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave + as.factor(COUNTY_FIPS), data = I_df, weights = weight_pooled)
print(summary(bi_mask_ind))

bi_mask_ind_raw <- lm(Norm_Masks_Used ~ log_prev_week_inc_rate + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, data = I_df, weights = weight_pooled)
...
coef_dem_raw <- summary(bi_mask_dem_raw)$coefficients["log_prev_week_inc_rate", c("Estimate", "Std. Error")]
coef_rep_raw <- summary(bi_mask_rep_raw)$coefficients["log_prev_week_inc_rate", c("Estimate", "Std. Error")]
coef_ind_raw <- summary(bi_mask_ind_raw)$coefficients["log_prev_week_inc_rate", c("Estimate", "Std. Error")]
...
```{r}
bi_contact_dem <- lm(num_cc_nonhh ~ log_prev_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave + as.factor(COUNTY_FIPS), data = D_df, weights = weight_pooled)
print(summary(bi_contact_dem))

bi_contact_dem_raw <- lm(num_cc_nonhh ~ log_prev_week_inc_rate + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, data = D_df, weights = weight_pooled)

bi_contact_rep <- lm(num_cc_nonhh ~  log_prev_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave + as.factor(COUNTY_FIPS), data = R_df, weights = weight_pooled)
print(summary(bi_contact_rep))

bi_contact_rep_raw <- lm(num_cc_nonhh ~ log_prev_week_inc_rate + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, data = R_df, weights = weight_pooled)

bi_contact_ind <- lm(num_cc_nonhh ~  log_prev_week_inc_rate + county_mask_mandate + COUNTY_RUCC_2013 + male + educ_group + resp_yob + resp_hhsize + r_race + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave + as.factor(COUNTY_FIPS), data = I_df, weights = weight_pooled)
print(summary(bi_contact_ind))

bi_contact_ind_raw <- lm(num_cc_nonhh ~ log_prev_week_inc_rate + cityNY +`cityBay Area` +cityPhiladelphia + cityPhoenix +cityBoston + cityAtlanta + wave, data = I_df, weights = weight_pooled)
...
coef_dem_raw <- summary(bi_contact_dem_raw)$coefficients["log_prev_week_inc_rate", c("Estimate", "Std. Error")]
coef_rep_raw <- summary(bi_contact_rep_raw)$coefficients["log_prev_week_inc_rate", c("Estimate", "Std. Error")]
coef_ind_raw <- summary(bi_contact_ind_raw)$coefficients["log_prev_week_inc_rate", c("Estimate", "Std. Error")]
